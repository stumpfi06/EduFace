    ...
    <div class="interface-body">
        ...
        <div v-if="!state.isEditing && !state.isCreating" class="filters">
            <div class="filter-dropdown">
                <button @click="toggleDropdown" class="filter-dropdown-button">Nach Klassen filtern</button>
        ...
        <table class="student-table" v-if="!state.isEditing && !state.isCreating">
            ...
            <tbody>
                <tr v-for="student in displayedStudents" :key="student.docId">
                    ...
                    <td>
                        ...
                        <button @click="handleAddFace(student)" class="action-button">
        ...
        <div class="pagination" v-if="!state.isEditing && !state.isCreating">
            ...
            <span v-if="state.searchMode"> Page {{ state.searchPage }} of {{ totalSearchPages }} </span>
            <span v-else> Page {{ state.currentPage }} </span>
            <button
                @click="nextPage"
        ...
        />
        <CreateSchueler v-if="state.isCreating" @close="state.isCreating = false" @student-created="handleStudentCreated" />
    </div>
</template>
...
import { getUserRole } from '@/firebase/users.ts'
...
export default {
    ...
    setup(props, { emit }) {
        ...
        const handleAddFace = async (student) => {
            try {
                const newSid = await addFace();
                console.log(student.docID)
                console.log(student)
                if (student.docId) {
                    await updateStudentSid(student.docId, newSid);
                    const updateStudentInArray = (arr) => arr.map(s =>
                        s.docId === student.docId ? { ...s, sid: newSid } : s
                    );
                    state.students = updateStudentInArray(state.students);
                    state.searchResults = updateStudentInArray(state.searchResults);
                    console.log("Updated SID for docId", student.docId, "from", student.sid, "to", newSid);
                } else {
                    console.error("Student hat keine docId:", student);
                }
            } catch (error) {
        ...
        const loadStudents = async (reset = false, direction = 'next') => {
            ...
            if (direction === 'next') {
                ...
                state.students = schueler.map(s => {
                    console.log('Schüler hinzugefügt (loadStudents - next):', s);
                    return { ...s };
                });
                ...
                state.students = schueler.map(s => {
                    console.log('Schüler hinzugefügt (loadStudents - prev):', s);
                    return { ...s };
                });
                state.lastVisible = lastDoc
        ...
        const handleStudentCreated = (newStudentWithDocId) => {
            state.isCreating = false;
            console.log('Schüler hinzugefügt (handleStudentCreated):', newStudentWithDocId);
            state.students = [...state.students, newStudentWithDocId];
            if (state.searchMode) {
                state.searchResults = [...state.searchResults, newStudentWithDocId];
            }
        };
        ...
        const performAdvancedSearch = async () => {
            ...
            querySnapshot.forEach((doc) => {
                const studentData = { docId: doc.id, sid: doc.data()?.sid, ...doc.data() };
                console.log('Schüler hinzugefügt (performAdvancedSearch - from Firestore):', studentData);
                allStudents.push(studentData);
            })
            if (state.searchQuery.trim() !== '') {
                const fuse = new Fuse(allStudents, {
                    keys: ['Name.Nachname', 'Name.Vorname', 'Katalognummer', 'sid'],
                    threshold: 0.3,
                })
                const results = fuse.search(state.searchQuery.trim())
                state.searchResults = results.map((result) => {
                    console.log('Schüler hinzugefügt (performAdvancedSearch - Fuse result):', result.item);
                    return result.item;
                })
            } else {
                state.searchResults = allStudents.map(student => {
                    console.log('Schüler hinzugefügt (performAdvancedSearch - allStudents):', student);
                    return student;
                });
            }
        ...
        watch(
            [() => state.searchQuery, () => state.selectedClasses],
            async ([newQuery, newClasses]) => {
