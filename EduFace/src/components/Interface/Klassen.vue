<template>
  <div class="interface-body">
    ...
    <div class="pagination" v-if="!state.isEditing && !state.isCreating">
      <button class="btn-pagination" @click="prevPage" :disabled="state.currentPage === 1">
        Previous
      </button>
...
export default {
  ...
  setup() {
    ...
    const loadKlassen = async (reset = false, direction = 'next') => {
      ...
      const { klassen, lastDoc } = await getKlassen(
        state.sortKey,
        state.sortOrder,
        state.pageSize,
